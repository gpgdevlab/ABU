...
# Uma lista simples para guardar os nomes dos itens coletados
...
# Função para verificar se o jogador tem um item específico (ex: uma chave)
func tem_item(nome_do_item: String) -> bool:
	return itens.has(nome_do_item)

# Função para remover um item (ex: ao usar a chave)
